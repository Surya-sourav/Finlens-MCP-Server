...
/**
 * Registers every QuickBooks tool on the given MCP server. Extracted from
 * src/index.ts so that BOTH the single-tenant stdio entry point and the
 * multi-tenant HTTP transport (Core/http/handle-mcp-request.ts) register the
 * exact same tool set through one definition. Per-tool CRUD gating still
 * happens inside RegisterTool via the QUICKBOOKS_DISABLE_* env vars.
 */
